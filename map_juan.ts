...
const FALLBACK_HUARIQUES: Huarique[] = [
  {
    ...
    descripcion: 'Ceviche cl├ísico de carretilla preparado al momento con pesca fresca del d├¡a, abundante lim├│n piurano y choclo desgranado.',
    tipoComida: 'Marina',
    coordenadas: {
      type: 'Point',
    ...
    votosNoExiste: [],
    resenas: [
      { usuarioId: 'correo1@gmail.com', usuarioNombre: 'Sandra P.', comentario: 'El ceviche de carretilla m├ís fresco y picante de la zona. Se agota r├ípido.', calificacion: 5, fecha: '2026-06-16T12:00:00Z' },
      { usuarioId: 'correo2@gmail.com', usuarioNombre: 'Carlos M.', comentario: 'Buen├¡simo y generoso, aunque la cola avanza lento.', calificacion: 4, fecha: '2026-06-18T14:30:00Z' }
    ],
    ratingPromedio: 4.5,
    numResenas: 2
  },
  {
    ...
    descripcion: 'Tradicionales brochetas de coraz├│n a la parrilla marinados en aj├¡ panca y especias, acompa├▒ados de papas doradas y choclo tierno.',
    tipoComida: 'Criolla',
    coordenadas: {
      type: 'Point',
      coordinates: [-77.0222, -12.1495] // Barranco
    },
    horario: 'Lun - S├íb: 6:00 PM - 11:30 PM',
    votosExiste: ['correo1@gmail.com', 'correo3@gmail.com'],
    votosNoExiste: ['correo4@gmail.com'],
    resenas: [
      { usuarioId: 'correo1@gmail.com', usuarioNombre: 'Sandra P.', comentario: 'El sabor del carb├│n es perfecto y el aj├¡ carretillero pica riqu├¡simo.', calificacion: 5, fecha: '2026-06-15T19:00:00Z' }
    ],
    ratingPromedio: 5.0,
    numResenas: 1
  },
  {
    ...
    votosNoExiste: [],
    resenas: [
      { usuarioId: 'gaston@huarique.pe', usuarioNombre: 'Gast├│n Acurio', comentario: 'Espectacular flambeado en wok. El jugo es perfecto para mojar las papas.', calificacion: 5, fecha: '2026-06-14T13:00:00Z' }
    ],
    ratingPromedio: 5.0,
    numResenas: 1
  },
  {
    ...
    votosNoExiste: [],
    resenas: [
      { usuarioId: 'correo3@gmail.com', usuarioNombre: 'Miguel T.', comentario: 'Cl├ísica hamburguesa de la t├¡a. Un salvavidas de madrugada.', calificacion: 4, fecha: '2026-06-10T23:30:00Z' }
    ],
    ratingPromedio: 4.0,
    numResenas: 1
  },
  {
    _id: 'fallback-5',
    nombre: 'Cevicher├¡a El Arrecife',
    descripcion: 'El ceviche de carretilla m├ís fresco y picante de la zona.',
    tipoComida: 'Marina',
    coordenadas: {
      type: 'Point',
...
const CATEGORIES = ['Todos', 'Marina', 'Criolla', 'Fusi├│n', 'Comida R├ípida', 'Poller├¡as', 'Chifas', 'Amaz├│nica', 'Picanter├¡as', 'Postres'];
...
export default function MapShell({ isConnected, setIsConnected, user, onAuthClick }: MapShellProps) {
  ...
  useEffect(() => {
    const fetchHuariques = async () => {
      ...
      try {
        ...
        setError('Sin conexi├│n al backend NestJS. Mostrando huariques de respaldo locales.');
  ...
  useEffect(() => {
    ...
    // A├▒adir control de zoom arriba a la izquierda
  ...
  // Resetear estados de rese├▒a al cambiar de huarique seleccionado
  useEffect(() => {
  ...
  // Actualizar marcadores cuando cambian los huariques filtrados o la selecci├│n
  useEffect(() => {
      ...
      marker.bindPopup(`
        <div class="leaflet-popup-content-inner" style="font-family: 'Outfit', sans-serif; min-width: 155px;">
          ...
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
            ...
            <span style="font-size: 11px; color: #eab308; font-weight: 600;">Ôÿà ${rating > 0 ? rating.toFixed(1) : 'S/R'}</span>
          </div>
          <div style="font-size: 11px; display: flex; align-items: center; gap: 4px; color: #64748b;">
            <span>­ƒÆ¼ ${totalReviews} rese├▒as comunitarias</span>
          </div>
        </div>
      `, {
        closeButton: false,
  ...
  // Centrar el mapa con animaci├│n suave al seleccionar un huarique
  useEffect(() => {
          ...
          {isRegisterMode ? (
            <div className="registration-container">
              <div className="registration-header-row">
                ...
                <button className="registration-close-btn" onClick={() => setIsRegisterMode(false)}>Ô£ò</button>
              </div>
              ...
              <form onSubmit={handleRegisterSubmit} className="registration-form">
                ...
                <div className="form-group">
                  ...
                  <textarea
                    placeholder="Cu├®ntanos qu├® lo hace tan especial..."
                ...
                <div className="form-group">
                  <label className="form-label">Ubicaci├│n Geogr├ífica *</label>
                  <div className={`coordinates-status-box ${regCoordinates ? 'success' : 'pending'}`}>
                    {regCoordinates ? (
                      ...
                      <div className="coords-helper">
                        <span className="pulse-icon">­ƒôì</span>
                  ...
                  >
                    ...
                    <div className="huarique-tags-row">
                      ...
                      {h.ratingPromedio !== undefined && h.ratingPromedio > 0 ? (
                        <span className="huarique-rating-badge">Ôÿà {h.ratingPromedio.toFixed(1)} ({h.numResenas || 0})</span>
                      ) : (
                        <span className="huarique-rating-badge" style={{ background: 'rgba(100, 116, 139, 0.08)', color: '#64748b', borderColor: 'transparent' }}>Sin rese├▒as</span>
                      )}
                    </div>
        ...
        <section className="map-shell">
            ...
            <div className="detail-overlay" style={{ zIndex: 1010 }}>
              ...
              <div style={{ display: 'flex', gap: '8px', marginBottom: '12px', flexWrap: 'wrap' }}>
                ...
                {selectedHuarique.ratingPromedio !== undefined && selectedHuarique.ratingPromedio > 0 && (
                  <span className="huarique-rating-badge">Ôÿà {selectedHuarique.ratingPromedio.toFixed(1)} ({selectedHuarique.numResenas} rese├▒as)</span>
                )}
              ...
              {/* 1. Sistema de Validaci├│n de Existencia de la Comunidad */}
              <div className="validation-container">
                <div className="validation-header">
                  ...
                  <span className="validation-badge" style={{ backgroundColor: existenciaPercent >= 70 ? '#10b981' : '#f59e0b' }}>
                    {existenciaPercent >= 70 ? 'Verificado' : 'Bajo revisi├│n'}
                  </span>
                </div>
                ...
                <div className="validation-info-row">
                  <span>{existenciaPercent}% de confirmaci├│n ({votosTotales} votos)</span>
                </div>
                <div className="validation-buttons">
                  ...
                  >
                    ­ƒæì S├¡ existe ({votosExisteCount})
                  </button>
                  <button 
                    onClick={() => handleVoteExistence(selectedHuarique._id, false)} 
                    className={`val-btn val-btn-no ${haVotadoNoExiste ? 'active' : ''}`}
                    title="Reportar que el huarique no existe o cerr├│"
                  >
                    ­ƒæÄ No existe ({votosNoExisteCount})
                  </button>
                </div>
              </div>
              ...
              <div className="reviews-container">
                ...
                <div className="reviews-scroller">
                  {(!selectedHuarique.resenas || selectedHuarique.resenas.length === 0) ? (
                    <p className="no-reviews-text">A├║n no hay rese├▒as de este huarique. ┬íS├® el primero en calificarlo!</p>
                  ) : (
                ...
                {/* Formulario de env├¡o de rese├▒as */}
                {user ? (
                  <form onSubmit={(e) => handleAddReviewSubmit(e, selectedHuarique._id)} className="review-form">
                    ...
                    <div className="review-input-row">
                      <input
                        type="text"
                        required
                        placeholder="Escribe tu opini├│n sobre la comida, precio, etc."
                  ...
                  <div className="review-login-prompt" onClick={onAuthClick}>
                    ­ƒöÆ Inicia sesi├│n para calificar y dejar una rese├▒a
                  </div>
                )}
              </div>
            </div>
          ...
          {isRegisterMode && (
            <div className="map-center-coordinates register-help-banner" style={{ zIndex: 1010 }}>
              {regCoordinates 
                ? '­ƒôì Ubicaci├│n seleccionada. Arrastra el pin rojo si necesitas reajustarlo.' 
                : '­ƒæê Haz clic en cualquier lugar del mapa para fijar el huarique.'
              }
            </div>
          )}
        </section>
